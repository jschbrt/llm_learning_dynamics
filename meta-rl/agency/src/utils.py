...
class AgentMemoryOptim:
    ...
    def insert_cues(self, idx, trials_idx, block_idx, cues):
    ...
    def insert_data(self, idx, trials_idx, forced_type, reward_block_type, rewards, counter_rewards, regrets, actions, optimal_actions, log_policy_as, baselines, entropy, policy):
        """
    ...
    def get_data(self, idx):
        ...
        if self.feedback == 'full':
            if self.batch_size == 1:
                data = torch.cat((cues, prev_cues, actions, rewards, counter_rewards)).view(1,-1)
            else:
    ...
    def get_padded_data(self, idx, trials_idx, block_idx):
        ...
        if self.with_time:
            norm_time = idx / self.episode_length
            self.time_matrix[:,idx, :] = norm_time
            time_matrix = self.time_matrix[mask1d].view(self.batch_size, -1, 1)
            data = torch.cat((data, time_matrix.clone()), dim=2)
    ...
    def get_simulated_data(self):
        ...
        cues = self.cues.cpu()    
        saved_data = (self.block_idx, self.forced_type, self.reward_block_type, self.trials_idx, rewards, counter_rewards, self.regrets, self.optimal_actions, actions, self.entropy, self.value_fn, self.policy)
...
class LogProgress:
    ...
    def save_test(self, eps_idx, simulated_data):
            ...
            block_idx, forced_type, reward_block_type, trials_idx, rewards, forgone_rewards, regrets, opt_actions, actions, entropy, value_fn, policy = [d.detach() for d in saved_data]
    ...
    def train_mean_free_cue(self, eps_idx, simulated_data, losses):
            ...
            # Train DF
            # Save the mean reward for free cue
            summed_cue = cues.sum(axis=2)
            idx = torch.where(summed_cue == 0)

            free_block_idx = block_idx[idx].view(self.batch_size, -1)
            free_forced_type = forced_type[idx].view(self.batch_size, -1)
            free_reward_block_type = reward_block_type[idx].view(self.batch_size, -1)
            ...
            free_forced_type = free_forced_type[0,:]
            free_reward_block_type = free_reward_block_type.mean(dim=0)
            ...
            df = pd.DataFrame({
                ...
                'block_reward_type': free_reward_block_type, # percentage of high reward blocks
                'block_forced_type': free_forced_type, # should be 0 always
    ...
    def save_train(self):
        ...
        train_df = self.train_df.groupby(['plot_idx', 'trials_idx', 'block_idx', 'block_forced_type']).mean(numeric_only=True).reset_index()
    ...
    def plot_train(self, dfs):
        ...
        tr_plt = train_df.groupby(['plot_idx', 'block_forced_type']).mean(numeric_only=True)
        

        self.writer.add_scalar('tr.p_opt_a.fo', tr_plt.perc_opt_actions.loc[self.temp_plot_idx, 1], self.temp_plot_idx)
        self.writer.add_scalar('tr.regrets.fo', tr_plt.regrets.loc[self.temp_plot_idx, 1], self.temp_plot_idx)
        self.writer.add_scalar('tr.rewards.fo', tr_plt.rewards.loc[self.temp_plot_idx, 1], self.temp_plot_idx)

        self.writer.add_scalar('tr.p_opt_a.fr', tr_plt.perc_opt_actions.loc[self.temp_plot_idx, 0], self.temp_plot_idx)
        self.writer.add_scalar('tr.regrets.fr', tr_plt.regrets.loc[self.temp_plot_idx, 0], self.temp_plot_idx)
        self.writer.add_scalar('tr.rewards.fr', tr_plt.rewards.loc[self.temp_plot_idx, 0], self.temp_plot_idx)
