"""
Contains the code of the A2C model
"""
...
class Transformer(nn.Module):
    """
    Transformer network used in the A2C Class. Similar to LSTM
    """
    def __init__(self, batch_size, input_size, hidden_size, is_cuda, num_layers, n_heads, writer=None) -> None:
        super().__init__()
        self.writer = writer
        ...
        self.input_laver = nn.Linear(in_features=input_size, out_features=hidden_size)
    ...
    def forward(self, x, idx):
        # x is assumed to be of shape (batch_size, sequence_length, feature_size=7)
        #x = self.input_layer(x)
        # padding mask has to be of shape (batch_size, sequence_length) -> reduce with all over dim=2

        # create padding mask
        #padding_mask = torch.full(x.shape, False, dtype=torch.bool, device=self.device)
        #padding_mask[:, idx+1:, :] = True
        #padding_mask = padding_mask.all(dim=2)

        x = x[:, :idx+1, :]
        x = self.input_laver(x)
        output = self.transformer_encoder(x)
        # take only the prediction of the values realted to the encoding of current cue (idx) 
...
class LSTM(nn.Module):
    ...
    def __init__(self, batch_size, hidden_size, input_size, is_cuda, writer=None) -> None:
        """
