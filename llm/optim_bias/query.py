...
parser.add_argument('--engines', nargs='+', default=['debugging'])
...
for engine in engines:
    ...
    start_run = 0
    if os.path.exists(f'./data/{engine}'):
        start_run = len(os.listdir(f'./data/{engine}'))
    for run in range(start_run, num_runs):
        ...
        df.to_csv(f'./data/{engine}/run_' + str(run) + '.csv')