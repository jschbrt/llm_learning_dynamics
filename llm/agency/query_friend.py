...
from env import PartialTaskForcedOnly as PartialTask
from utils import AgentMemoryOptim, LogProgress

# Run
parser = argparse.ArgumentParser()
parser.add_argument('--engines', nargs='+', default=['debug'])
parser.add_argument('--num_runs',  default=8) # 24 (part) * 3 (sess) = 72 
    ...
    for run in range(start_run, num_runs):
        ...
        for block_idx in range(bandit.number_of_blocks):
            ...
            if bandit.forced_type == 1:
                instructions += f"Half of the time you visit the casino, you can play. The other half, a friend of yours is playing and you can see the rewards for their chosen slot machine. Your friend is trying to help you determine the most valuable machine. "
            trials_left = f"Your goal is to maximize the total amount of points you receive in all the {bandit.number_free} visits you can play. "
            ...
            for trials_idx in range(bandit.total_trials_block):
                ...
                if forced == 1:
                    ...
                    history += f"- On visit {trials_idx+1} your friend played Machine {ac} and got a reward of {rew} point.\n"
    
                else:
                    history += f"- On visit {trials_idx+1} you played Machine {ac} and received a reward of {rew} point.\n"
