    ...
    for run in range(start_run, num_runs):
        ...
        for block_idx in range(bandit.number_of_blocks):
            ...
            if bandit.forced_type == 1:
                instructions += f"Half of the time you visit the casino, you can freely play, the other half you have to play a certain slot machine and will see the rewards without receiving them. "
            trials_left = f"Your goal is to maximize the total amount of points you receive in all {bandit.number_free} visits you can freely play."
            ...
            for trials_idx in range(bandit.total_trials_block):
                ...
                if forced == 1:
                    ...
                    history += f"- On visit {trials_idx+1} you had to play Machine {ac} and saw a reward of {rew} point.\n"
